...
echo "    docker exec -i leantime-db mariadb -u root -p<LEANTIME_DB_ROOT_PASSWORD> < /path/to/leantime_db_dump.sql"
echo "    docker exec -i kimai-db mariadb -u root -p<KIMAI_DB_ROOT_PASSWORD> < /path/to/kimai_db_dump.sql"
