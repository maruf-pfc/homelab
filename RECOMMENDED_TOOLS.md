...
## 📂 Table of Contents
1. [Media & Automation (Arr Stack)](#1-media--automation-arr-stack)
2. [Dashboards & Portals](#2-dashboards--portals)
3. [Network, DNS & Ingress](#3-network-dns--ingress)
...
| App / Tool | Category | Port | Footprint | Tech Stack & Database | Storage | Description |
| :--- | :--- | :---: | :---: | :--- | :--- | :--- |
| **Prometheus** | Metrics TSDB | `9090` | 🟡 Moderate | `Go` / `Built-in TSDB` | SSD + HDD | Time-series metrics collection engine. *(RAM depends on metric retention)* |
| **Grafana** | Visualization | `3005` | 🟢 Lightweight | `Go / React` / `SQLite` | SSD | Analytics & dashboard platform for visualizing Prometheus metrics. |
| **Uptime Kuma** | Status Monitoring | `3001` | 🟢 Lightweight | `Node.js / Vue` / `SQLite` | SSD | Fancy self-hosted monitoring tool supporting HTTP, Ping, and push alerts. |
| **Node Exporter** | Host Metrics | `9100` | ⚡ Ultra-Light | `Go` / `Stateless` | Host | Prometheus hardware exporter for CPU, RAM, Disk, and Network metrics. |
| **cAdvisor** | Container Metrics | `8083` | 🟢 Lightweight | `Go` / `Stateless` | Host | Resource usage and performance analyzer for running Docker containers. |
...
## 5. Storage, Cloud & Photo Management
...
## 6. Security, Password Management & Auth
...
## 9. Sysadmin, DevOps & Container Utilities

| App / Tool | Category | Port | Footprint | Tech Stack & Database | Storage | Description |
| :--- | :--- | :---: | :---: | :--- | :--- | :--- |
| **Portainer** | Container Manager | `9000` | ⚡ Ultra-Light | `Go / Angular` / `BoltDB` | SSD | GUI tool for managing Docker containers, stacks, volumes, and networks. |
...
| **IT-Tools** | Admin Utilities | `8091` | ⚡ Ultra-Light | `Vue.js (Client-side JS)` / `Stateless (No DB)` | Stateless | Handy online tools for developers and sysadmins (JWT, IP, Generators). |
